   ...
   * @post 			max_policies()+=num_policies_to_add
	*/
	void increase_num_policies(size_type num_policies_to_add){
		...
		for(size_type i=cur_max; i<(cur_max+num_policies_to_add); ++i)
			free_id_set_.insert(i);
	...
	//can add a module to decrease the number of policies
...
/**-------------------Policy-level methods (affect an individual policy)*/
   ...
   * @post 			all samples adhere to policy and sample-wide constraints
	...
	*/
	void copy_policy(Policy& from_p, Policy& to_p) {
		to_p.value() = from_p.value(); 
		to_p.max_size() = from_p.max_size();
		to_p.max_samples() = from_p.max_samples();
  	} 

	/* Add a sample to a policy*/
	void add_sample(Policy p, sample_info_type si){
		p.add_sample(si);
	}

	/* Add a sample to a policy*/
	void add_sample(Policy p, sample_value_type sv){
		p.add_sample(sv);
	}

	/** Create a new policy using the samples from another policy that meet the policy constraints
	
  	void copy_samples_that_meet_policy(Policy& from_p, Policy& to_p) {
		mark_if_meets_policy(from_p);
		for (auto it = from_p.samples_start(); it != from_p.samples_end(); ++it)
			this->add_sample(to_p,from_p.sample(it).value());
  	} */
	...
	* @post			sample_id may no longer be valid after deletion
	...
	bool delete_sample(Policy& p, sample_value_type s){
		assert(is_valid(p));
		size_type pid  = p.get_id();
...
/*------------Getters and Setters for policies */
   ...
   * @post 			p.value() = pv
	*/
	...
	/**Returns number of samples in a policy**/
	size_type max_samples_allowed(Policy& p) {
		return max_num_samples(p);
	}
  ...
  class Policy:private totally_ordered<Policy> {
   ...
   policy_value_type& value() const{
	...
	void add_sample(Sampler::sample_info_type& si){
		fetch_samples().add_node(Point(),si);
	}

	void add_sample(Sampler::sample_value_type& sv){
	...
	void mark_if_meets_policy(){
		fetch().value_.mark_if_meet_constraints();
	}

	void mark(sample_info_type& si,bool meets){
		si.meets_policy_ =  meets;
	}

	void enforce_policy(){
		fetch().value_.enforce_policy();
	}*/
   ...
   sample_iterator samples_start(){
	...
	Sample sample(Sampler::sample_iterator s){
		return (*s);
	}

   sample_info_type& sample(size_type sid){
		return sample(node(sid));
   }

	sample_info_type& sample(Sampler::Sample s){
		return s.value(); //changed from fetch_samples().s.value();
   }
